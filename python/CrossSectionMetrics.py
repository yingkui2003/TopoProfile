...
with arcpy.da.SearchCursor(profile3D, ["ProfileID", "SHAPE@", "SHAPE@LENGTH"]) as cursor:
    ...
    for row in cursor: ##Loop for each line
        ...
        heightArr = np.array(valley_heights)

        valid_heights = heightArr[heightArr > 0]        
        if len(valid_heights) > 1:
            total_area = sum(valid_heights) * max(LengthfromStart) * 0.5
        else:
            total_area = sum(valid_heights) * max(LengthfromStart)
        ...
        #arcpy.AddMessage((cross_area, v_area))
        ...
        #arcpy.AddMessage((line_length,height))
        #WH_ratio = line_length / (height + 0.001) ## to avoid the division by zero
        WH_ratio = line_length / height ## to avoid the division by zero
        #arcpy.AddMessage(float(WH_ratio))
        ...
        if len(R2_list) > 0:
            ...
            VWDR_m_list.append(-999)
            VWDR_n_list.append(-999)
            VWDR_r2_list.append(-999)                    
